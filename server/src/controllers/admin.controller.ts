...
import { ApiResponse, DatabaseError, InternalServerError, NotFoundError, StandardError, UnauthorizedError } from '../util'
import { removeFromCloudinary, uploadOnCloudinary } from '../util/Cloudinary'
import { AdminService } from '../services/admin.service'
import { changePasswordSchema, updateProfileSchema } from '../validators/admin.validator'
...
export class AdminController {
    static async updateProfile(req: CustomRequest, res: Response) {
        ...
        const updatedUser = await AdminService.updateUserProfile(userId, validateData)
    ...
    static async removeAvatar(req: CustomRequest, res: Response) {
        try {
            ...
            res.status(200).json(ApiResponse(req, res, 200, 'Avatar removed successfully', {}))
        } catch (error) {
            if (error instanceof StandardError) {
                throw error
            }

            throw new InternalServerError('An unexpected error occurred while removing avatar')
        }
    }
    ...
    static getProfile(req: CustomRequest, res: Response) {
        try {
            ...
            throw new InternalServerError('An unexpected error occurred while fetching profile')
        }
    }
}
