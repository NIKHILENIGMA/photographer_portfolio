...
import { DatabaseError, InternalServerError, NotFoundError, StandardError } from '../util'

export class PhotoService {
    static async addImageToPhoto(imagePath: string, userId: string, addPhotoData: addPhotoData): Promise<string> {
        try {
    ...
    static async updatePhotoDetails(photoId: string, payload: updatePhotoData): Promise<string> {
        try {
            ...
            throw new InternalServerError('An unexpected error occurred while updating the photo')
        }
    }

    static async updateImage(photoId: string, newFilePath: string): Promise<string> {
        try {
            const photo = await this.getPhotoByPhotoId(photoId) // Check if the photo exists
            ...
            throw new InternalServerError('An unexpected error occurred while updating the image')
        }
    }
    ...
    static async deletePhoto(photoId: string): Promise<string> {
        try {
            ...
            throw new InternalServerError('An unexpected error occurred while deleting the photo')
        }
    }
